...
import { estadoDoTexto, rotuloDaExecucao } from "./executado";
import { conjuntoDeColunas, localDoModelo } from "./modelos";
 ...
 *
 * Há três jogos de colunas — convênio, particular e clínica —, e uma planilha
 * por clínica. Por isso tudo aqui recebe o `modelo`: a mesma tradução, aplicada
 * ao conjunto de colunas de cada um.
 */

const COLUNAS_DO_CONJUNTO = {
  unimed: COLUNAS_UNIMED,
  outros: COLUNAS,
  particular: COLUNAS_PARTICULAR,
  clinica: COLUNAS_CLINICA,
};
...
function montar(modelo) {
  return [
    // Primeira coluna: é a caixa que se marca ao conferir o mês, e conferir é
    // percorrer a planilha de cima a baixo marcando — no fim da linha ela
    // obrigava a atravessar a largura toda em cada paciente.
    { label: "Executado", key: "executado", tipo: "execucao" },
    { label: "Data", key: "dataCirurgia", tipo: "data" },
    { label: "Hora do lançamento", key: "horaLancamento", tipo: "horaLancamento" },
    ...COLUNAS_DO_CONJUNTO[conjuntoDeColunas(modelo)].map((c) => ({
      label: c.label,
      key: c.key,
      tipo: tipoDaColuna(c),
...
/**
 * Onde cada coluna esperada está na planilha, segundo o cabeçalho dela.
 *
 * A leitura de volta era por posição, e posição muda: bastou "Procedimento
 * complementar" mudar de lugar para a planilha antiga — a que ainda está no
 * Drive, com a ordem de ontem — passar a ser lida uma casa fora, e o cirurgião
 * viraria procedimento complementar na volta.
 *
 * Aqui a coluna é achada pelo nome, como as larguras em `lib/sheets.js` já são.
 * Devolve `null` quando falta uma coluna que é lida: aí é planilha que não dá
 * para interpretar, e não ler é melhor do que ler errado — a reescrita da
 * sincronização repõe o cabeçalho e a leitura volta a funcionar no ciclo
 * seguinte.
 *
 * Coluna derivada que falta não impede nada: ela é conta do app, nunca é lida
 * de volta. Isso é o que deixa a planilha continuar sendo lida na primeira
 * sincronização depois de uma coluna dessas ser criada — e sem isso, estrear a
 * coluna Local custaria um ciclo de edições não lidas.
 */
export function ordemDoCabecalho(cabecalhoNaPlanilha, modelo = "convenio") {
  const naPlanilha = (cabecalhoNaPlanilha || []).map((t) => normalizarTexto(t));
  const campos = camposDaLinha(modelo);
  const indices = campos.map((c) => naPlanilha.indexOf(normalizarTexto(c.label)));
  const faltaUmaQueImporta = indices.some((i, n) => i < 0 && campos[n].tipo !== "derivada");
  return faltaUmaQueImporta ? null : indices;
}

/** A linha da planilha reordenada para a ordem que o resto daqui espera. */
export function linhaNaOrdem(linha, ordem) {
  return ordem.map((i) => linha[i]);
}
...
function textoDaCelula(entry, campo) {
  ...
  if (campo.tipo === "execucao") return rotuloDaExecucao(entry[campo.key]);
...
function valorDaCelula(campo, texto) {
  ...
  // Três estados, e não sim/não: célula que não é nenhum dos três volta `null`
  // e é recusada, em vez de virar "não" e dar a cirurgia por não feita.
  if (campo.tipo === "execucao") return estadoDoTexto(limpo);
 ...
 *
 * Devolve { registro, campos, recusadas, emBranco }: o registro já com as
 * alterações, os rótulos do que mudou, o que não deu para aceitar e se a linha
 * estava vazia.
 */
export function lerLinha(entry, linha, modelo = "convenio") {
  ...
  // Linha com ID e mais nada não é edição: é linha esvaziada à mão.
  //
  // Acontece de verdade — uma planilha duplicada para servir de base, com as
  // linhas antigas limpas: a coluna de ID é oculta, escapa da limpeza e fica
  // apontando para pacientes que estão vivos no app. Lida célula a célula, cada
  // uma dessas linhas apagaria o nome, o convênio, a guia e os procedimentos do
  // paciente, sem ninguém ter pedido.
  //
  // Vale a mesma regra da linha removida: apagar paciente é decisão do app, onde
  // há confirmação. A sincronização repõe a linha.
  const soTemId = camposDaLinha(modelo).every(
    (campo, coluna) => campo.key === "id" || !(linha[coluna] || "").toString().trim()
  );
  if (soTemId) return { registro: entry, campos: [], recusadas: [], emBranco: true };
...
export function registroDaLinha(linha, modelo = "convenio") {
  ...
  return normalizarRegistro({
    ...bruto,
    // A planilha não tem a coluna que diz de quem ela é — é o arquivo que diz.
    // Sem preencher aqui, a linha digitada à mão nasceria sem convênio ou sem
    // local e migraria sozinha para outra planilha na sincronização seguinte.
    ...(modelo === "particular" ? { convenio: "PARTICULAR" } : {}),
    ...(localDoModelo(modelo) ? { local: localDoModelo(modelo) } : {}),
    id: randomUUID(),
