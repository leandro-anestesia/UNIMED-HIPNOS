 ...
 * Devolve `null` quando falta alguma: aí é planilha que não dá para interpretar,
 * e não ler é melhor do que ler errado — a reescrita da sincronização repõe o
 * cabeçalho e a leitura volta a funcionar no ciclo seguinte.
 */
export function ordemDoCabecalho(cabecalhoNaPlanilha, modelo = "convenio") {
  ...
  const indices = cabecalhos(modelo).map((label) => naPlanilha.indexOf(normalizarTexto(label)));
