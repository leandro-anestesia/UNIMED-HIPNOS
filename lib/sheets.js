...
const IDX_EXECUTADO = INDICE_DO_EXECUTADO;
const IDX_ID = INDICE_DO_ID;
  ...
  for (const ano of anos) {
    ...
    (resposta.data.valueRanges || []).forEach((faixa, i) => {
      ...
      (faixa.values || []).forEach((linha) => {
        ...
        if (!id) {
          const novo = registroDaLinha(linha);
          if (novo) {
            novos.push(novo);
        ...
        const { registro, campos, recusadas: naoAceitas } = lerLinha(entry, linha);
