 ...
 *
 * Para mudar uma largura, mude o número; a sincronização seguinte aplica em
 * todos os meses de uma vez.
 */
const LARGURAS = {
  Data: 95,
  "Hora do lançamento": 130,
  Prontuário: 105,
  Paciente: 350,
  Convênio: 160,
  "Nº da Guia": 115,
  ...
  Cirurgião: 250,
  Anestesista: 420,
  "Anestesista (carimbo)": 390,
  Executado: 95,
  "Procedimento complementar": 235,
  Observação: 260,
};
...
/** Colunas de valor curto, que ficam melhor no meio da célula. */
const CENTRALIZADAS = [
  "Data",
  "Hora do lançamento",
  "Prontuário",
...
/** Aplica nesta aba as larguras e o alinhamento definidos acima. */
function requestsDeLayout(sheetId) {
  const requests = [];

  HEADERS.forEach((label, i) => {
    if (i === IDX_ID) return; // coluna técnica: fica oculta, largura não importa
    ...
    // Alinhamento só onde ele é escolhido: sem isto, o padrão do Google vale,
    // e é ele que deixa número à direita e texto à esquerda.
    if (!CENTRALIZADAS.includes(label)) return;
    requests.push({
      repeatCell: {
